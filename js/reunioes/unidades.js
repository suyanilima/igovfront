...
function rotuloPeriodicidadeReunioes(periodicidade){
  const valor=String(periodicidade || '').trim();
  if(!valor) return 'Reuniões com periodicidade personalizada';
  const plurais={
    semanal:'semanais',
  ...
  const normalizada=valor.toLocaleLowerCase('pt-BR');
  return `Reuniões ${plurais[normalizada] || normalizada}`;
}
...
function renderUnidadesReuniao(){
  ...
  CODIGOS_UNIDADES_FIXAS.forEach(codigo=>{
    ...
    if(periodicidade) periodicidade.textContent=rotuloPeriodicidadeReunioes(configuracao?.periodicidade || FREQUENCIAS_REUNIAO[codigo]);
  });
